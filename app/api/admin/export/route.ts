...
export async function GET(request: NextRequest) {
  ...
  const { data, error } = await supabase
    ...
    .select(`
      ...
      result_types(score_key, label, description, reasons, failure_pattern, seven_day_plan, action_free, action_low_cost, action_fastest, service_title, service_description, affiliate_links)
    `)
    .eq("id", id)
    .single();
