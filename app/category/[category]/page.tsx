...
const categoryMeta: Record<string, { title: string; description: string }> = {
  personality: {
    title: "性格診断",
    description: "あなたの性格タイプや強みがわかる診断を集めました。",
  },
  sidejob: {
    title: "副業診断",
    description: "あなたに向いている副業タイプがわかる診断を集めました。",
  },
  strength: {
    title: "強み診断",
    description: "あなたの強みや得意分野がわかる診断を集めました。",
  },
};
...
export default async function CategoryPage({ params }: PageProps) {
  const { category } = await params;
  const meta = categoryMeta[category];
  const filtered = diagnoses.filter((d) => d.category === category);
    ...
    return (
      <main className="px-4 py-12">
        <div className="mx-auto max-w-3xl rounded-3xl bg-white p-10 text-center shadow-sm ring-1 ring-slate-200">
          ...
          <Link
            href="/"
            className="mt-6 inline-flex rounded-full bg-slate-900 px-6 py-3 text-sm font-semibold text-white transition hover:opacity-90"
          >
            ホームに戻る
          </Link>
        </div>
    ...
    <main className="px-4 py-12">
      <div className="mx-auto max-w-4xl">
        <div className="mb-8">
          ...
          <p className="mt-4 mb-2 text-sm font-semibold text-rose-500">
            CATEGORY
          </p>
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          {filtered.map((d) => (
            ...
            >
              ...
              <div className="mt-4 text-sm font-semibold text-rose-500">
                診断をはじめる →
              </div>
            </Link>
          ))}
            ...
            {Object.entries(categoryMeta)
              ...
              .map(([key, value]) => (
                <Link
                  key={key}
                  href={`/category/${key}`}
                  className="rounded-full bg-white px-4 py-2 text-sm font-medium text-slate-600 ring-1 ring-slate-200 transition hover:bg-rose-50 hover:text-rose-500"
                >
                  {value.title}
                </Link>
              ))}
